...
let createRows = computed(()=>{
    let getRows = props.rows.filter(function (item){
        if(item.create){
            return item
        }
    });
...
onMounted(()=>{
    ...
    let count = {};
    form.value.permissions.forEach(function (item){
        let c = item.name.lastIndexOf('_')
        let result = item.name.substring(c + 1);
        count[result] = 1;
        permValue.value[item.name] = true;
        tableValue.value[item.table] = true;
    });
})
...
function save(redirect=false){
    form.value.guard_name= "web";
    let onSubmit = useForm(form.value).submit('post', route('roles.update', props.record.id), {
        onSuccess: ()=>{
            form.value = {};
            back()
        }
...
layoutStore.setBreadcrumbs({
    ...
    label: "Edit Role #"+props.record.id
});

</script>
<template>
    <div class="grid px-6 mx-auto pt-6">
        <Header
            :title="'Edit Role #'+props.record.id"
        />
                                                    ...
                                                    <div class="flex items-center justify-between space-x-2 rtl:space-x-reverse">
                                                        <label @click.prevent="checkAllItems(!selectAll)"  class="filament-forms-field-wrapper-label inline-flex items-center space-x-3 rtl:space-x-reverse" for="data.select_all">
                                                            <button  role="switch" aria-checked="false" :class="{
                                                                     ...
                                                                     id="data.select_all"
                                                                     type="button"
                                                                     class="filament-forms-toggle-component relative inline-flex shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:border-main focus:ring-2 focus:ring-main disabled:opacity-70 disabled:cursor-not-allowed disabled:pointer-events-none border-gray-300 bg-gray-200 dark:bg-white/10">
                                            ...
                                            <div class="col-span-1" v-for="(item, index) in perm" :key="index">
                                                <div class="filament-forms-card-component p-6 bg-white rounded-xl border border-gray-300 dark:border-gray-600 dark:bg-gray-800 shadow-sm">
                                                    <div class="grid grid-cols-1 lg:grid-cols-2 filament-forms-component-container gap-6">
                                                        ...
                                                        <div class="col-span-full">
                                                            <fieldset class="filament-forms-fieldset-component rounded-xl shadow-sm border border-gray-300 p-6 dark:border-gray-600 dark:text-gray-200 text-primary-600">
                                                                ...
                                                                <div class="grid grid-cols-2 lg:grid-cols-2 xl:grid-cols-2  filament-forms-component-container gap-6">
                                                                    <div class="col-span-1" v-for="(getItem, key) in item" :key="key">
                                                                        <div class="filament-forms-field-wrapper">
                                                                            <div class="space-y-2">
                                                                                <div class="flex items-center justify-between space-x-2 rtl:space-x-reverse">
                                                                                    <label class="filament-forms-field-wrapper-label inline-flex items-center space-x-3 rtl:space-x-reverse" for="data.view_blog::author">
                                                                                        <input
                                                                                            id="data.view_blog::author"
                                                                                            type="checkbox"
                                                                                            v-model="permValue[getItem.name]"
                                                                                            @click="togglePerm(getItem, permValue[getItem.name])"
                                                                                            class="filament-forms-checkbox-component text-primary-600 transition duration-75 rounded shadow-sm focus:border-main focus:ring-2 focus:ring-main disabled:opacity-70 dark:bg-gray-700 dark:checked:bg-primary-600 border-gray-300 dark:border-gray-600 text-primary-600">
                                                                                        <span class="text-sm font-medium leading-4 text-gray-700 dark:text-gray-300 capitalize">
